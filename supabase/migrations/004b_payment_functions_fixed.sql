-- Payment and transaction management functions (RECOMMENDED VERSION)
-- Created: 2025-01-28
-- Updated: 2025-10-05
-- This is the FIXED and IMPROVED version - USE THIS ONE!

-- First, drop any existing conflicting functions
DROP FUNCTION IF EXISTS record_payment_transaction(UUID, UUID, VARCHAR, NUMERIC, VARCHAR, VARCHAR);
DROP FUNCTION IF EXISTS record_payment_transaction(UUID, UUID, VARCHAR, DECIMAL, VARCHAR, VARCHAR);
...
-- Function to record a payment transaction (using VARCHAR for flexibility)
...
BEGIN
  INSERT INTO payment_transactions (
    ...
    bounty_uuid,
    user_uuid,
    tx_type,  -- Using VARCHAR directly for flexibility
    amount,
    currency,
    tx_hash,
...
-- Function to get user payment history (using VARCHAR types for compatibility)
...
-- Drop trigger if exists and recreate (prevents conflicts)
...
-- Helper function to check transaction type validity
...
-- Helper function to check transaction status validity
